...
{ id: “c2”, nom: “Urbanisme & Travaux”, color: “#059669”, icon: “🏗️” },
...
try { if (typeof Notification !== “undefined” && Notification.permission === “granted”) new Notification(“🏛️ Mairie de Noailly”, { body: ${ev.title} — ${fmtRel(ev.date)} }); } catch(e) {}
...
{Array.from({length:off}).map((*,i)=><div key={“e”+i}/>)}
{Array.from({length:dim}).map((*,i)=>{ const d=i+1,isT=d===today.getDate()&&cM===today.getMonth()&&cY===today.getFullYear(),isSel=d===sd,de=evOn(d);
...
{upcoming.length===0?<div style={{ background:”#fff”,borderRadius:T.r,padding:24,textAlign:“center”,boxShadow:T.sh }}><div style={{ fontSize:32,marginBottom:8 }}>📅</div><div style={{ color:T.tm,fontSize:14 }}>Aucun événement à venir</div></div>
...
<Fl label="Type"><div style={{ display:“flex”,gap:8 }}>{[[“reunion”,“🤝 Réunion”],[“conseil”,“🏛️ Conseil”],[“rdv”,“📋 RDV”],[“autre”,“📌 Autre”]].map(([k,l])=><button key={k} onClick={()=>setType(k)} style={{ flex:1,padding:“8px 4px”,borderRadius:10,border:`2px solid ${type===k?T.primary:T.bd}`,background:type===k?T.primary+“0d”:”#fff”,fontSize:12,fontWeight:500,cursor:“pointer”,color:type===k?T.primary:T.tm }}>{l}</button>)}</div></Fl>
...
{fl.length===0?<div style={{ background:”#fff”,borderRadius:T.r,padding:24,textAlign:“center”,boxShadow:T.sh }}><div style={{ fontSize:32,marginBottom:8 }}>📄</div><div style={{ color:T.tm,fontSize:14 }}>{rp.length===0?“Aucun rapport”:“Aucun résultat”}</div></div>
...
<div style={{ width:44,height:44,borderRadius:12,background:(cm?.color||T.accent)+“14”,display:“flex”,alignItems:“center”,justifyContent:“center”,fontSize:22,flexShrink:0 }}>{r.type===“pv”?“📝”:r.type===“note”?“📌”:“📄”}</div>
...
return <div style={{ padding:16 }}><div style={{ background:”#fff”,borderRadius:T.r,padding:20,boxShadow:T.sh }}>
<Fl label="Type"><div style={{ display:“flex”,gap:8 }}>{[[“pv”,“📝 PV”],[“cr”,“📄 CR”],[“note”,“📌 Note”]].map(([k,l])=><button key={k} onClick={()=>setType(k)} style={{ flex:1,padding:“8px 4px”,borderRadius:10,border:`2px solid ${type===k?T.primary:T.bd}`,background:type===k?T.primary+“0d”:”#fff”,fontSize:13,fontWeight:500,cursor:“pointer”,color:type===k?T.primary:T.tm }}>{l}</button>)}</div></Fl>
...
{e.tel&&<a href={`tel:${e.tel}`} style={{ fontSize:12,color:T.primary,textDecoration:“none” }}>📞 {e.tel}</a>}
{e.email&&<a href={`mailto:${e.email}`} style={{ fontSize:12,color:T.primary,textDecoration:“none” }}>✉️ Email</a>}
</div>
</div>
